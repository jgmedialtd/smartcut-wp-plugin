import { m as markRaw, _ as _export_sfc, o as openBlock, l as createElementBlock, n as createBaseVNode, t as toDisplayString, u as normalizeClass, A as withModifiers, B as getDefaultExportFromCjs, D as commonjsGlobal, E as useI18n, G as userFriendlyFieldMap, H as getSeparators, I as localeParseFloat, r as resolveComponent, p as createVNode, J as block0 } from "./main-DUuuC60f.js";
const _sfc_main$1 = {
  name: "Drop",
  props: {
    requiredType: {
...
const Drop = /* @__PURE__ */ _export_sfc(_sfc_main$1, [["render", _sfc_render$1]]);
...
const _sfc_main = {
  ...
  data() {
    return {
      ...
      fieldNames: userFriendlyFieldMap,
      manualMapping: {
  ...
  computed: {
    ...
    someInvalid() {
      return Object.values(this.itemValidation).some((item) => {
        return Object.values(item).some((v) => v === false && Object.keys(item).some((field) => this.partFields.includes(field)));
      });
    },
    itemValidation() {
      const validation = {};
      this.items.forEach((p, i) => {
        ...
        for (const [key, value] of Object.entries(p)) {
          if (!this.partFields.includes(key)) continue;
    ...
    isSingleBandingChoice() {
      const keys = Object.keys(this.bandingChoices);
      return keys.length === 1 && this.bandingChoices[keys[0]].length === 0;
    },
    customFieldLabels() {
      return this.customFields.map((field) => field.label);
    }
  },
  mounted() {
    this.locale = this.options.locale;
    if (this.bandingChoices) {
      ...
      const combinations = [[]];
      keys.forEach((key) => {
        const values = this.bandingChoices[key];
      ...
      this.bandingChoiceCombinations = validCombinations;
    }
  },
  methods: {
    parse(files) {
      ...
      Papa.parse(file, {
        ...
        //parse numbers
        worker: true,
        complete: (results) => this.process(results),
        error: () => this.$emit("error")
      });
    },
    process(results) {
      this.userRows = results;
      this.userRows = this.userRows.data.filter((row) => row.some((cell) => cell));
      this.userHeaders = this.userRows[0];
      this.columnMap = {};
      for (let i = this.userHeaders.length; i--; ) {
        const header = this.userHeaders[i];
        const predictedType = this.predictHeadingType(header);
        if (!predictedType) {
          this.columnMap[i] = null;
          if (this.customFieldLabels.includes(header)) {
            this.columnMap[i] = "customData." + this.customFields.find((field) => field.label === header).id;
          }
        } else {
          this.columnMap[i] = predictedType;
        }
      }
    ...
    processColumnMap() {
      this.showDataMismatch = false;
      this.mappedHeaders = Object.values(this.columnMap).map((heading) => {
        if (!heading || heading === "N") {
          return null;
        }
        if (!this.partFields.includes(heading)) {
          this.showDataMismatch = true;
          return null;
        }
        return heading;
      });
      const duplicateHeadings = this.mappedHeaders.filter((heading, index) => this.mappedHeaders.indexOf(heading) !== index);
      const duplicateIndexes = duplicateHeadings.map((heading) => this.mappedHeaders.reduce(
    ...
    validate(key, value) {
      ...
      function dimensionValidation(v) {
        ...
        if (v === "l" || v === "w") return true;
        return false;
      }
      const validation = {
        banding: (v) => {
          v = this.stripSpacesAroundCommas(v);
          if (!v) return true;
          const individualBandingChoices = v.split(",");
          if (individualBandingChoices.every((val) => this.isValidBandingCombination(val))) return true;
          const invalid = individualBandingChoices.filter((val) => !this.isValidBandingCombination(val));
          console.log("The following banding choices are invalid", invalid);
          console.log("Valid choices...");
          console.table(Array.from(this.bandingChoiceCombinations));
          return false;
        },
        orientationLock: (v) => {
          return dimensionValidation(v);
        }
      };
      if (key in validation) return validation[key](value);
      return null;
    },
    parseItemValue(key, value) {
      const parsing = {
        bandingChoices: (v) => {
          v = this.stripSpacesAroundCommas(v);
          if (!v)
          ...
          let parsed = v.split(",").map((b) => b.toLowerCase());
          if (this.isSingleBandingChoice) {
            const keys = Object.keys(this.bandingChoices);
            parsed = parsed.map((v2) => v2 === "1" ? keys[0] : "");
            const bandingOptions = {
              y1: parsed[0] ? { [parsed[0]]: true } : null,
              y2: parsed[1] ? { [parsed[1]]: true } : null,
              x1: parsed[2] ? { [parsed[2]]: true } : null,
              x2: parsed[3] ? { [parsed[3]]: true } : null
            };
            ...
            Object.entries(this.bandingChoices).forEach(([bandingChoiceKey], bandingChoiceIndex) => {
              bandingOptions.y1[bandingChoiceKey] = parsed[0].split("|")[bandingChoiceIndex];
              bandingOptions.y2[bandingChoiceKey] = parsed[1].split("|")[bandingChoiceIndex];
              bandingOptions.x1[bandingChoiceKey] = parsed[2].split("|")[bandingChoiceIndex];
              bandingOptions.x2[bandingChoiceKey] = parsed[3].split("|")[bandingChoiceIndex];
            });
            return bandingOptions;
          }
        }
      };
      if (key in parsing) return parsing[key](value);
    },
    populateColumnData() {
      const mappedData = this.userRows.map((row) => {
        const rowData = {};
        this.userHeaders.forEach((h, i) => {
          if (this.duplicateIndexes.flat().includes(i)) {
            return rowData[this.columnMap[i]] = "???";
          }
          rowData[this.columnMap[i]] = row[i];
        });
        return rowData;
      });
      this.items = mappedData;
    },
    transformCustomData(item) {
      let customData = {};
      for (let key in item) {
        if (key.startsWith("customData.")) {
          let newKey = key.slice("customData.".length);
          customData[newKey] = item[key];
        }
      }
      return customData;
    },
    convertPartDataToInputData() {
      return this.items.map((item, itemIndex) => {
        const inputShapeData = {
          l: this.units === "decimal" ? localeParseFloat(item.l) : item.l,
          w: this.units === "decimal" ? localeParseFloat(item.w) : item.w,
          t: this.units === "decimal" ? localeParseFloat(item.t) : item.t,
          q: item.q,
          orientationLock: item.orientationLock,
          //cater for Y/1 value
          name: item.name,
          material: item.material,
          bandingOptions: this.itemValidation[itemIndex].banding ? this.parseItemValue("bandingChoices", item.banding) : null,
          units: this.units,
          customData: this.transformCustomData(item)
        };
        return inputShapeData;
      });
    },
    importParts() {
      const inputData = this.convertPartDataToInputData();
      if (!(inputData == null ? void 0 : inputData.length)) return;
      this.$emit("import", inputData);
    },
    isValidBandingCombination(value) {
      return this.bandingChoiceCombinations.has(value.toLowerCase());
    }
  }
};
const _hoisted_1 = { id: "import-file" };
function _sfc_render(_ctx, _cache, $props, $setup, $data, $options) {
  const _component_Drop = resolveComponent("Drop");
  return openBlock(), createElementBlock("div", _hoisted_1, [
    createVNode(_component_Drop, {
      label: $setup.t("Drop CSV file here"),
      "required-type": "text/csv",
      onDrop: $options.parse
    }, null, 8, ["label", "onDrop"])
  ]);
}
if (typeof block0 === "function") block0(_sfc_main);
const ImportCSV = /* @__PURE__ */ _export_sfc(_sfc_main, [["render", _sfc_render]]);
export {
