import { _ as _export_sfc, o as openBlock, k as createElementBlock, l as createBaseVNode, t as toDisplayString, q as normalizeClass, E as withModifiers, G as useI18n, H as userFriendlyFieldMap, I as getSeparators, J as localeParseFloat, r as resolveComponent, n as createVNode, K as block0 } from "./main-VTn5Qv8T.js";
const _sfc_main$1 = {
  name: "Drop",
  props: {
    requiredType: {
  ...
  mounted() {
    this.events.forEach((eventName) => {
      document.body.addEventListener(eventName, this.eventHandler);
    });
  },
  unmounted() {
    this.events.forEach((eventName) => {
      document.body.removeEventListener(eventName, this.eventHandler);
    });
  },
  methods: {
    readFileAsync(file) {
...
const _sfc_main = {
  ...
  computed: {
    ...
    someInvalid() {
      return Object.values(this.itemValidation).some((item) => {
        return Object.values(item).some(
          (v) => v === false && Object.keys(item).some(
            (field) => this.partFields.includes(field)
          )
        );
      });
    },
    someDuplicates() {
      var _a, _b;
      return ((_b = (_a = this.duplicateIndexes) == null ? void 0 : _a.flat()) == null ? void 0 : _b.length) > 0;
    },
    itemValidation() {
      const validation = {};
      this.items.forEach((p, i) => {
        ...
        for (const [key, value] of Object.entries(p)) {
          if (!this.partFields.includes(key))
            continue;
  ...
  methods: {
    ...
    process(results) {
      ...
      this.userRows = this.userRows.data.filter(
    ...
    processColumnMap() {
      ...
      this.mappedHeaders = Object.values(this.columnMap).map(
        (heading) => {
      ...
      const duplicateHeadings = this.mappedHeaders.filter(
        (heading, index) => this.mappedHeaders.indexOf(heading) !== index
      );
      const duplicateIndexes = duplicateHeadings.map(
        (heading) => this.mappedHeaders.reduce(
    ...
    importParts() {
      ...
      if (!(inputData == null ? void 0 : inputData.length))
        return;
...
if (typeof block0 === "function")
  block0(_sfc_main);
const Import = /* @__PURE__ */ _export_sfc(_sfc_main, [["render", _sfc_render]]);
export {
  Import as default
};
//# sourceMappingURL=Import-Bh8iEWam.js.map
