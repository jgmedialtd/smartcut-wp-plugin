...
const St = {
  ...
  key: 0,
  class: "menu groups"
}, Dt = { class: "row table-heading" }, Yt = { class: "cell" }, Ut = { class: "cell" }, Nt = ["onClick"], Mt = { class: "cell center" }, Ot = { class: "cell text clickable" }, Pt = { class: "number quantity cell" }, Tt = ["value", "onInput"], Bt = { class: "cell" }, Ft = ["aria-label", "onClick"], Rt = {
  key: 0,
  id: "groups-canvas"
}, Vt = {
  key: 1,
  ...
  class: "all issues"
}, Et = 400, K = 300, T = 4, qt = /* @__PURE__ */ mt({
  ...
  setup(Z, { expose: tt, emit: et }) {
    const N = et, { inputs: i, inputSaw: Ht } = bt(), k = Z, a = X(null), _ = X(null), q = X(4), M = X([]), B = X(!1);
    let S = null, m = null, v = null;
    const st = V(() => "34px auto 60px 30px"), G = V(() => a.value?.shapeIds?.length ? a.value.shapeIds.map((o) => {
    ...
    function at() {
      ...
      const p = gt().on("start", function(t, l) {
        ...
        const g = m.invert(t.x) - r, w = v.invert(t.y) - u, d = $(g, w, l), R = U(this);
        R.select("rect.part-rect").attr("x", m(d.x)).attr("y", v(d.y + l.w)).classed("collision", !d.snapped).classed("valid", d.snapped), R.selectAll("text.dim-label").attr("x", () => m(d.x) + 2).attr(
          "y",
          ($t, vt) => vt === 0 ? v(d.y + l.w) + 10 : v(d.y) - 2
        ), R.select("text.part-index").attr("x", m(d.x + l.l / 2)).attr("y", v(d.y + l.w / 2) + 4);
      }).on("end", function(t, l) {
        U(this).attr("cursor", "grab");
        const g = m.invert(t.x) - r, w = v.invert(t.y) - u, d = $(g, w, l);
        d.snapped && (a.value.setShapePosition(l.autoId, d.x, d.y), l.x = d.x, l.y = d.y), D(() => C());
      });
      n.call(p);
    }
    function nt(o) {
    ...
    const ot = (o) => {
      ...
      F(), M.value = [], i.inputUserGroups.value.length = 0;
    }, F = () => {
      a.value = null, _.value = null, N("clear-highlight");
    }, W = () => {
      a.value && a.value.validate(i.inputShapes.value);
    }, A = () => {
      M.value = _t(i.inputUserGroups.value, i.inputShapes.value, !1);
    };
    O(() => i.inputShapes.value, () => {
      ...
      _.value = null, N("clear-highlight"), !B.value && (B.value = !0, W(), A(), D(() => {
        B.value = !1, C();
      }));
    }, { deep: !0 }), O(() => G.value, () => {
      a.value && (A(), W(), D(() => C()));
    ...
    const it = () => {
      ...
      if (o > -1 && a.value) {
        ...
        _.value = null, N("clear-highlight"), D(() => C());
      }
    };
    return tt({
      deselect: F,
      ...
      return y(), I("div", St, [
        ...
        f(i).inputUserGroups.value.length ? (y(), I("div", Xt, [
          c("button", {
            type: "button",
        ...
        c("div", null, [
          f(i).inputUserGroups.value.length ? (y(), I("div", {
            key: 0,
            ...
            (y(!0), I(ft, null, xt(f(i).inputUserGroups.value, (n, r) => (y(), I("div", {
              key: n.autoId,
              class: z(["row", {
              ...
              c("div", Bt, [
                c("button", {
                  ...
                  onClick: E((u) => lt(n.autoId), ["stop"])
                }, [
                  Y(s, { icon: ["fass", "trash"] })
                ], 8, Ft)
              ]),
              a.value?.autoId === n.autoId ? (y(), I("div", {
                key: 0,
                class: "group vis",
                style: j({
...
}), Zt = /* @__PURE__ */ kt(qt, [["__scopeId", "data-v-dae2a212"]]);
export {
  Zt as default
};
