import { g as ref, G as useLocalStorage, h as computed, E as shallowRef, S as triggerRef, i as watch } from "./vendor-vue-Dk_GuyMk.js";
import { I as InputStock, a as InputShape, u as updateInputNumberFormat, b as Issue, t as totalQuantity, s as stockMatch, S as Stock, m as mapOrientation, c as Shape, g as getStockGrainSummary, r as recreateUserGroups, d as sentenceCase, e as unplacedShapeTally, f as getStockStack, h as Saw, i as recreateStockList, j as recreateShapeList, k as recreateSegmentList, l as recreateCutList, n as recreateOffcuts, o as sorting, p as convertNumberFormat } from "./components-DNQps9NE.js";
import { p as purify, m as lookup } from "./vendor-Bl85u5aa.js";
let instance$1 = null;
function createInputs(options = { numberFormat: "decimal" }) {
  ...
  const totalInputShapeLines = computed(() => {
    return inputShapes.value.reduce((total, s) => {
      if (!(s == null ? void 0 : s.multiEdit)) return total + 1;
      return total;
    }, 0);
  });
  ...
  const totalInputStockLines = computed(() => {
    return inputStock.value.reduce((total, s) => {
      if (!(s == null ? void 0 : s.multiEdit)) return total + 1;
      return total;
    }, 0);
  ...
  const createShapeList = (options2 = {
    ...
    const sourceInputs = options2.inputShapesOverride ?? inputShapes.value;
  ...
  const validateInputStock = (saw = null) => {
    var _a;
    const issues = [];
    ...
    for (const s of inputStock.value) {
      issues.push(...s.validate(saw, index));
      index++;
    }
    return issues;
  };
  const validateInputShapes = (options2 = null) => {
    ...
    const issues = [];
    const shapesToValidate = inputShapesOverride ?? inputShapes.value;
    if (!shapesToValidate.length) {
      new Issue({
        category: ["part"],
    ...
    const totalQ = totalQuantity(shapesToValidate);
    if (!totalQ) {
      new Issue({
        category: ["part"],
        message: "errors.validation.no_parts",
      ...
      shapesToValidate.forEach((s) => new Issue({
        category: ["part"],
        message: "errors.validation.no_parts",
        field: [["q"]],
        item: s
    ...
    let index = 0;
    for (const s of shapesToValidate) {
      issues.push(...s.validate({
        inputStockList: inputStock.value,
        index,
        stockType: saw !== null ? saw.stockType ?? null : null,
        partTrim,
        useInventory,
        saw
      }));
    ...
    stockMatch(shapesToValidate, (_a = inputs == null ? void 0 : inputs.inputStock) == null ? void 0 : _a.value, saw);
    const stockMatchIssues = shapesToValidate.flatMap((s) => s.issues);
  ...
  return {
    ...
    totalInputShapes,
    totalInputStock,
    totalInputShapeLines,
    totalInputStockLines,
    reset,
    clear,
    clearIssues,
    updateInputs,
...
function useInputs(options = { numberFormat: "decimal" }) {
  if (!instance$1) instance$1 = createInputs(options);
  return instance$1;
}
const notices = ref([]);
...
function createState() {
  ...
  const currentCutIndex = ref(null);
...
function useResults() {
  ...
  const activeShapes = computed(() => {
    if (!state.activeStockId.value) return [];
    return getShapes(state.activeStockId.value);
  });
  const activeStock = computed(() => {
    var _a;
    if (!state.activeStockId.value) return null;
    if (!((_a = state.stockList.value) == null ? void 0 : _a.length)) return null;
    return getStock(state.activeStockId.value);
  });
  const activeCuts = computed(() => {
    var _a;
    if (!activeStock.value || !((_a = state.cutList.value) == null ? void 0 : _a.length)) return [];
    const cuts = getCuts(activeStock.value.id);
    return cuts;
  });
  const activeSegments = computed(() => {
    var _a;
    if (!activeStock.value || !((_a = state.segmentList.value) == null ? void 0 : _a.length)) return [];
    return getSegments(state.activeStockId.value);
  });
  const usedStock = computed(() => {
    var _a;
    if (!((_a = state.stockList.value) == null ? void 0 : _a.length)) return [];
    return getUsedStock(state.stockList.value);
  });
  const addedShapes = computed(() => {
    return state.shapeList.value.filter((shape) => shape.added);
  });
  const unplacedShapes = computed(() => {
    var _a;
    if (!((_a = state.shapeList.value) == null ? void 0 : _a.length)) return [];
    return state.shapeList.value.filter((shape) => !shape.added);
  });
  const unplacedShapeTally$1 = computed(() => {
    var _a;
    if (!((_a = state.shapeList.value) == null ? void 0 : _a.length)) return [];
    return unplacedShapeTally(state.shapeList.value);
  });
  const uniqueShapes = computed(() => {
    var _a;
    if (!((_a = state.shapeList.value) == null ? void 0 : _a.length)) return [];
    return state.shapeList.value.filter((shape) => !shape.duplicate);
  });
  ...
  const uniqueStock = computed(() => {
    var _a;
    if (!((_a = state.stockList.value) == null ? void 0 : _a.length)) return [];
    return state.stockList.value.filter((stock) => stock.used && !stock.duplicate);
  });
  ...
  const activeCutsArea = computed(() => {
    var _a;
    if (!((_a = state.cutList.value) == null ? void 0 : _a.length)) return 0;
    return activeCuts.value.reduce((total, cut) => total + cut.getArea(), 0);
  });
  const activeStockArea = computed(() => {
    var _a;
    if (!((_a = state.stockList.value) == null ? void 0 : _a.length) || !activeStock.value) return 0;
    return activeStock.value ? activeStock.value.getArea() : 0;
  });
  const activeShapeArea = computed(() => {
    var _a;
    if (!((_a = state.shapeList.value) == null ? void 0 : _a.length) || !activeShapes.value) return 0;
    return activeShapes.value ? activeShapes.value.reduce((total, s) => total + s.getArea(), 0) : 0;
  });
  const activeCutType = computed(() => {
    var _a;
    if (!((_a = state.cutList.value) == null ? void 0 : _a.length) || !activeStock.value) return null;
    return activeStock.value.cutType;
  });
  const stackedStock = computed(() => {
    var _a;
    if (!((_a = state.stockList.value) == null ? void 0 : _a.length)) return [];
    const stacked = state.stockList.value.filter((stock) => stock.used && (stock.stack === false || typeof stock.stack === "number"));
    return stacked;
  });
  const activeStockStackCount = computed(() => {
    var _a;
    if (!((_a = state.stockList.value) == null ? void 0 : _a.length)) return 0;
    return getStockStack(activeStock.value, state.stockList.value);
  });
  ...
  const reset = () => {
    state.saw.value = null;
    state.shapeList.value.length = 0;
    state.stockList.value.length = 0;
    state.cutList.value.length = 0;
    state.segmentList.value.length = 0;
    state.offcuts.value.length = 0;
    state.unusableShapes.value.length = 0;
    state.currentCutIndex.value = null;
  };
  const clear = () => {
    state.saw.value = null;
    state.activeStockId.value = null;
    state.activeShape.value = null;
    state.metadata.value = null;
    state.shapeList.value = [];
    state.stockList.value = [];
    state.cutList.value = [];
    state.segmentList.value = [];
    state.offcuts.value = [];
    state.currentCutIndex.value = null;
    state.unusableShapes.value = [];
  };
  ...
  const updateShapes = () => {
    triggerRef(state.shapeList);
  };
  const updateStock = () => {
    triggerRef(state.stockList);
  };
  const updateFromResult = (result) => {
    ...
    state.metadata.value = result.metadata;
    state.unusableShapes.value = result.unusableShapes || [];
    try {
      if (result == null ? void 0 : result.saw) {
        state.saw.value = new Saw(
          result == null ? void 0 : result.saw,
          (_b = result == null ? void 0 : result.saw) == null ? void 0 : _b.stockType
        );
      }
      state.stockList.value = recreateStockList({
        saw: state.saw.value,
      ...
      state.shapeList.value = recreateShapeList({
        shapeList: result.shapeList,
        stockList: state.stockList.value,
        preventAutoRotation: true,
        skipValidation: true
      });
      state.cutList.value = recreateCutList({
        cutList: result.cutList,
        stockList: state.stockList.value
      });
      state.segmentList.value = recreateSegmentList({
        segmentList: result.segmentList,
        stockList: state.stockList.value
      });
      state.offcuts.value = recreateOffcuts({
      ...
      triggerRef(state.stockList);
      triggerRef(state.shapeList);
      triggerRef(state.cutList);
      triggerRef(state.segmentList);
      state.activeStockId.value = ((_d = (_c = getUsedStock(state.stockList.value)) == null ? void 0 : _c[0]) == null ? void 0 : _d.id) || null;
    } catch (e) {
  ...
  const getStock = (id) => {
    var _a;
    if (!((_a = state.stockList.value) == null ? void 0 : _a.length)) return null;
    return state.stockList.value.find((s) => s.id === id);
  };
  const getShapes = (stockId) => {
    var _a;
    if (!((_a = state.shapeList.value) == null ? void 0 : _a.length)) return [];
    return state.shapeList.value.filter((s) => {
      var _a2;
  ...
  const getSegments = (stockId) => {
    var _a;
    if (!((_a = state.segmentList.value) == null ? void 0 : _a.length)) return [];
    const segments = state.segmentList.value.filter((s) => {
      var _a2;
      return (((_a2 = s == null ? void 0 : s.stock) == null ? void 0 : _a2.id) ?? (s == null ? void 0 : s.stockId)) === stockId;
    });
    return segments;
  ...
  const getCuts = (stockId) => {
    var _a;
    const cuts = (_a = state == null ? void 0 : state.cutList) == null ? void 0 : _a.value;
    ...
    return cuts.filter((c) => {
      ...
      var _a2, _b;
      return ((_a2 = a == null ? void 0 : a.guillotineData) == null ? void 0 : _a2.order) - ((_b = b == null ? void 0 : b.guillotineData) == null ? void 0 : _b.order);
    });
  };
  const getUsedStock = (stockList) => {
    if (!(stockList == null ? void 0 : stockList.length)) return [];
    return stockList.filter((stock) => stock.used === true);
  };
  const setActiveStockId = (id) => {
    if (state.activeStockId.value === id) return;
    state.activeShape.value = null;
    state.activeStockId.value = id;
  };
  const createTestData = () => {
    state.saw.value = new Saw({
      bladeWidth: 1,
    ...
    state.cutList.value = recreateCutList({
      cutList: [
        { stockId: "1.0", x1: 0, x2: 2440, y1: 500.5, y2: 500.5, dimension: "l", direction: "lr", isTrim: false, isGuillotine: true },
        { stockId: "1.0", x1: 500.5, x2: 500.5, y1: 0, y2: 500, dimension: "w", direction: "bt", isTrim: false, isGuillotine: true },
        { stockId: "1.0", x1: 901.5, x2: 901.5, y1: 0, y2: 500, dimension: "w", direction: "bt", isTrim: false, isGuillotine: true },
        { stockId: "1.0", x1: 501, x2: 901, y1: 400.5, y2: 400.5, dimension: "l", direction: "lr", isTrim: false, isGuillotine: true }
      ],
      stockList: state.stockList.value
    });
    state.offcuts.value = recreateOffcuts({
  ...
  instance = {
    ...
    unusableShapes: state.unusableShapes,
    currentCutIndex: state.currentCutIndex,
...
function useProductPricing(settings, dependencies) {
  ...
  const setPrice = (type, price) => {
    ...
    if ((mapping == null ? void 0 : mapping.hidden) && inputs[mapping.hidden]) {
      log(["setting hidden price", mapping.hidden, price]);
  ...
  const calculateBasePrice = (resultData) => {
    ...
    let productPrice = (product == null ? void 0 : product.price) || getCurrentVariationPrice(true) || 0;
    ...
    const pricingStrategies = {
      ...
      part_area: (data) => {
        ...
        log([`part area price in square ${unitType} is ${partArea} (part area) * ${productPrice} (product price) = ${partAreaPrice}`]);
        let totalPrice2 = partAreaPrice;
        if ((settings == null ? void 0 : settings.enable_offcut_pricing) && (data == null ? void 0 : data.offcuts)) {
          ...
          const sellableOffcutAreaSquareUnits = sellableOffcutArea / divisor;
          const sellableOffcutPrice = sellableOffcutAreaSquareUnits * productPrice;
          setPrice("offcutArea", sellableOffcutPrice);
          log([`calculated sellable offcut area as ${sellableOffcutAreaSquareUnits} square ${unitType}, with price ${sellableOffcutPrice}`]);
          totalPrice2 += sellableOffcutPrice;
        }
        log([`calculated total price as ${totalPrice2}`]);
        if (isQuantityPricing()) {
          const totalArea = totalPrice2 / productPrice;
  ...
  const calculateExtras = (resultData) => {
    const {
      isExtraEnabled,
      isMachiningEnabled,
      isSurchargeEnabled,
  ...
  const calculatePrice = async (resultData) => {
    ...
    const extrasPrice = calculateExtras(resultData);
    totalPrice += extrasPrice;
    if (isQuantityPricing()) {
      const stockQuantity = resultData.metadata.totalUsedStock;
      const customPriceIncludingExtras = totalPrice / stockQuantity;
      setPrice("custom", customPriceIncludingExtras);
    } else {
...
//# sourceMappingURL=composables-_fF_hxOz.js.map
