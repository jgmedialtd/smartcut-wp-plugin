import { h as K, M as ue, a4 as vt, a5 as fe, a0 as de } from "./InputUserGroup-pBQzFMX3.js";
import { J as pe } from "./vendor-jszip-3jx27sm9.js";
import { o as me, F as ge, i as _e, j as he, m as be, b as ye, g as q, k as ke, u as we, l as ve, p as Pe, n as Ce } from "./EcommerceCalculator-D4REKTU7.js";
import { b as Se, d as xe } from "./i18n-DbggOnBy.js";
import { t as Fe } from "./i18n-cONG3r0U.js";
import { E as Ae, a as U } from "./vendor-jspdf-4Nu5WJZF.js";
import { d as Ee, l as Le, o as ct, c as $t, m as yt, u as lt, a as Te, b as kt, j as jt, F as $e, C as wt, s as N, Q as je, x as ze, I as Oe, G as qe, R as Be } from "./vendor-vue-D6ZfZC0P.js";
function ut(e) {
...
function zt(e) {
  ...
  for (const l of u)
    l?.variations ? (r.labels.push(...Object.keys(l.options)), Object.values(l.variations).forEach((c) => {
      ...
      const f = ut(c);
      r.pricing[f] = parseFloat(l.price), l.sku && (r.codes ??= {}, r.codes[f] = l.sku);
...
function Pt(e, r, u) {
  ...
  if (!c) return null;
  const o = c.trim().startsWith("=") ? c.trim().substring(1).trim() : c.trim(), f = Ne(u), y = Array.isArray(e.sheets) && e.sheets.length > 0 ? e.sheets : void 0;
  try {
    if (y) {
      let s = 0;
      for (const a of y) {
  ...
  const u = (t) => {
    ...
    switch (m) {
      ...
      default: {
        ...
        for (const [P, w] of h) {
          const C = t.stock?.find((j) => j.id === P);
          if (!C || !C.cost) continue;
          const F = C.l * C.w;
          if (F === 0) continue;
          const B = w.reduce((j, z) => j + z.l * z.w * (z.q || 1), 0);
          d += B / F * K({ v: C.cost }) * b;
        }
    ...
    if (P === "part_area" && e?.enable_offcut_pricing && t?.offcuts) {
      const j = V(e.unit_system, "area"), z = f(t, j, d);
      o("offcutArea", z), B += z;
    }
    if (g())
      if (P === "cut_length") {
        const j = V(e.unit_system, "length");
        m(t.metadata.totalCutLength / j, 2);
      } else if (P === "part_area") {
        const j = V(e.unit_system, "area");
...
function Mt(e) {
  return "extras" in e && e.extras ? ue.map((r, u) => ({ label: Bt[u], value: vt(e.extras?.banding?.sides?.[r], "-") })).filter(({ value: r }) => r !== "-").map(({ label: r, value: u }) => `${r}: ${u}`).join(`
`) || "-" : Ge.map((r, u) => ({ label: Bt[u], value: vt(e.banding?.[r], "-") })).filter(({ value: r }) => r !== "-").map(({ label: r, value: u }) => `${r}: ${u}`).join(`
`) || "-";
}
function Nt(e) {
  return fe.map((r, u) => ({ label: Ke[u], value: vt(e.extras?.finish?.faces?.[r] ?? e.finish?.faces?.[r], "-") })).filter(({ value: r }) => r !== "-").map(({ label: r, value: u }) => `${r}: ${u}`).join(`
`) || "-";
}
const Je = (e, r, u) => {
  const l = [];
...
const Ze = (e, r, u, l = (c) => c.toString()) => {
  ...
  __name: "WordPress",
  setup(e) {
    const r = wt(() => import("./Configurator-DmeNiMG3.js")), u = wt(() => import("./ObjectViewer-D4FhhG99.js")), l = wt(() => import("./Notices-D8aIr_Or.js")), { inputShapes: c } = ye(), { addNotice: o } = we(), f = {}, y = {}, S = N(null);
    let v, L, t;
    const s = {
      banding: null,
      finish: null
    ...
    const b = N(!1), h = N("manual"), P = N(""), w = je([]), C = ze(() => w.value), F = N(null), B = N(!0), j = N(null), z = N(!1), et = N(null);
    ...
    const O = (n) => {
      ...
      z.value = !0, ["checkout", "intro_text", "offcuts", "banding_key"].forEach((p) => J(p, "block")), ee() && J("pricing_table", "table"), n ? it(!0) : ft(), M();
    }, rt = (n = !0, i = !0) => {
      z.value = !1, ["checkout", "intro_text", "offcuts", "banding_key", "pricing_table"].forEach((_) => J(_, "none")), i === !0 ? Ct() : i === !1 && M(), it ? it() : n === !1 && ft();
    }, it = (n = !1) => {
      J("quantity", "inline-block"), f.quantity.readOnly = n;
    }, ft = () => {
      z.value !== !1 && (dt(1), J("quantity", "none"), f.quantity.readOnly = !0);
    }, Ct = () => {
      if (!f.cart_button) {
      ...
      t = n.settings;
      const i = zt(q(window, ["smartcutConfig", "banding_data"])), p = zt(q(window, ["smartcutConfig", "finish_data"])), _ = ($) => {
        switch ($) {
          case "length":
      ...
      return new de(E, t.numberFormat || "decimal");
    }, Kt = () => {
      ...
      if (ve(t, ["enable", n]) && q(t, ["enable", n]) === !1)
        return !1;
      const i = q(s, [n]);
      ...
      const p = Ze(Fe, n, j.value, Gt).split(",")[1], _ = new pe();
      ...
      M(), n && (j.value = qe(n)), mt("smartcut-hardware-price", i ?? 0);
    };
    return Le(() => {
      ...
      h.value === "formula" && P.value ? (ct(), jt(lt(r), {
        key: 1,
        ref: "configurator",
        "format-price": R,
        url: P.value,
      ...
      h.value === "manual" || h.value === "formula" && P.value ? (ct(), jt(ke, {
        key: 2,
        ref_key: "calculator",
        ref: et,
