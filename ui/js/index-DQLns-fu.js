import { d as f, o as v, c as b, g as Z, b as A, x as o } from "./vendor-vue-D6ZfZC0P.js";
const C = {
  // Arrows & chevrons
  ...
  "folder-tree": { stroke: '<rect x="3" y="4" width="9" height="6"/><rect x="14" y="14" width="7" height="6"/><path d="M7 10V17H14"/>' },
  ...
  ...B,
  ...Object.fromEntries(Object.entries(D).map(([t, e]) => [u(t), e]))
}, R = {
  "info-circle": "info",
...
Object.fromEntries(
  Object.entries(p).map(([t, e]) => [e, t])
);
const F = ["google", "youtube", "stripe", "wordpress"], a = (t) => Object.keys(t).map(u);
a(l), a(n), a(L), a(k), a(g), a(x), a(w), a(y), a(m), a(H), [...F];
const d = (t, e) => `<svg xmlns="http://www.w3.org/2000/svg" viewBox="${t}" width="1em" height="1em" fill="currentColor" aria-hidden="true">${e}</svg>`, r = {
  google: d(
    "0 0 488 512",
...
function J(t) {
  K && !i.has(t) && (i.add(t), console.warn(`[icons] no glyph for "${t}" — add it to icons/src/glyphs or ALIASES`));
}
const M = '<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" width="1em" height="1em" aria-hidden="true" shape-rendering="geometricPrecision">';
function Q(t) {
  if (!t) return "";
  if (t in r) return r[t];
  const e = P[G(t)];
  return e ? e.solid ? `${M}<g fill="currentColor" stroke="none">${e.solid}</g></svg>` : `${M}<g fill="none" stroke="currentColor" stroke-width="${W}" stroke-linejoin="round" stroke-linecap="round">${e.stroke}</g></svg>` : (J(t), "");
}
const U = ["innerHTML"], e1 = /* @__PURE__ */ f({
  ...
  setup(t) {
    const e = t, c = o(() => {
      ...
      const h = e.icon;
      return Array.isArray(h) ? h[h.length - 1] ?? "" : typeof h == "string" ? h : "";
    }), s = o(() => Q(c.value)), V = o(() => c.value ? `ck-${c.value}` : null);
    return (h, X) => s.value ? (v(), b("span", {
      key: 0,
      class: Z(["ck-icon", V.value]),
      innerHTML: s.value
    }, null, 10, U)) : A("", !0);
  }
});
