import { g as O, H as K, h as T, G as H, S as V, i as pt } from "./vendor-vue-D8CfTNeX.js";
import { I as tt, a as Y, u as et, b as W, t as at, s as mt, S as vt, m as gt, c as kt, g as St, r as st, d as Z, e as Lt, f as yt, h as rt, i as nt, j as ct, k as it, l as ot, n as lt, o as It, p as wt } from "./components-DaRBMrhg.js";
import { p as ut, l as bt } from "./vendor-DABu9zvL.js";
let D = null;
function Tt(t = { numberFormat: "decimal" }) {
  const g = O(null), r = () => {
    ...
    listenToStorageChanges: !1
  }), p = O(null), k = T(() => u.value.reduce((a, s) => s != null && s.q && s.q > 0 && !(s != null && s.multiEdit) ? a + s.q : a, 0)), n = T(() => c.value.reduce((a, s) => s != null && s.q && s.q > 0 && !(s != null && s.multiEdit) ? a + s.q : a, 0)), m = (a, s = "decimal") => {
  ...
  const E = () => {
    ...
    return e.length ? e : (mt(i, (U = F == null ? void 0 : F.inputStock) == null ? void 0 : U.value, s !== null ? s.stockType ?? null : null), i.flatMap((B) => B.issues));
  }, P = (a) => {
...
let X = null;
function Ct() {
  const t = O(null), g = O(null), r = O(null), _ = H([]), w = H([]), c = H([]), u = H([]), o = O([]), p = O(null), k = H([]), n = O(null), m = O(null);
  return {
    // All state
    jobId: t,
    saw: g,
...
function Gt() {
  ...
  const t = Ct(), g = T(() => t.activeStockId.value ? F(t.activeStockId.value) : []), r = T(() => {
    ...
    let e = I.value ? (C.value + v.value) / I.value : null;
    return e > 1 && (e = 1), e;
  }), b = () => {
...
function Mt(t, g) {
  const r = (n) => {
    var m;
    (m = g.log) == null || m.call(g, n);
  }, _ = (n) => {
    var m;
    return (m = g.error) == null || m.call(g, n), { success: !1, error: n };
  }, w = () => {
    for (const n of ht) c(n, 0);
  }, c = (n, m) => {
    r(["setting price", n, m]);
    const { inputs: q, elements: C, setInputValue: I, formatPrice: v } = g;
    if (isNaN(m)) throw new Error("Price must be a number");
    if (!xt(n)) return;
    const y = {
    ...
    y != null && y.hidden && q[y.hidden] && (r(["setting hidden price", y.hidden, m]), I(y.hidden, m)), y != null && y.visible && C[y.visible] && (C[y.visible].innerText = v(m));
  }, u = (n) => {
    const { product: m, getCurrentVariationPrice: q, isQuantityPricing: C, enableAddToCart: I, setCartQuantity: v } = g;
    let E = (m == null ? void 0 : m.price) || q(!0) || 0;
    ...
    const y = t.unit_system === "imperial" ? "feet" : "meters", $ = {
      ...
      part_area: (S) => {
        ...
        if (r([`calculated total price as ${G}`]), C()) {
          const M = G / E;
          return r([`setting cart quantity to ${M}`]), v(M, 2), 0;
        } else
          return G;
      },
      cut_length: (S) => {
    ...
    let $ = 0;
    if (m("banding") && n.metadata.bandingLengthByType) {
      const b = v(n.metadata.bandingLengthByType);
      c("banding", b), $ += b;
    }
    if (m("finish") && n.metadata.finishAreaByType) {
    ...
    const { getInputValue: m, setInputValue: q } = g, C = {
    ...
    if (Object.entries(C).forEach(([I, v]) => {
      q(I, v);
    }), m("smartcut-stock-summary") !== void 0) {
      const I = n.stock.map((v) => `${v.l}x${v.w}${v.t ? "x" + v.t : ""} [${v.q}]`).join(", ");
  ...
  return {
    calculatePrice: async (n) => {
      w();
      const { isQuantityPricing: m, selectedVariation: q, setVariationPrice: C, setCartQuantity: I } = g;
      if (!(n != null && n.jobId))
      ...
      const E = o(n);
      if (v += E, m()) {
        const y = n.metadata.totalUsedStock, R = v / y;
        c("custom", R);
      } else {
        if (!n.metadata.totalStockCost)
          return _("Total stock cost not returned for a multiple size product");
        r([`total cost is ${v}`]), c("custom", v);
      }
      return q.value && C(v), p(n), { success: !0, totalPrice: v };
    },
    resetPricing: w
  };
}
