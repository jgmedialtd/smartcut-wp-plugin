...
async function Mt() {
  const l = {
    ...
    l.main,
    l.errors
  ]), m = Object.keys(v), u = m.reduce((c, E) => ({
    ...c,
    [E]: {
      main: v[E],
      errors: O[E]
  ...
  return bt(ke), ke.use(yt).init({
    ...
    interpolation: {
      format: function(c, E) {
        return E === "capitalise" ? gt(c) : E === "sentenceCase" ? y(c) : c;
      }
    }
  });
}
...
const Yt = (l, v, O, m = (u) => u.toString()) => {
  ...
  const E = v.parts.map((p) => {
    var x;
    const _ = Oe.map((s) => p.banding[s] || "-").join(pe), t = Ie.map((s) => p.banding[s] || "-").join(pe);
    return [
      m(p.l),
      m(p.w),
      m(p.t) || "-",
  ...
  J(u, {
    ...
    headStyles: G,
    styles: X
  }), c = u.autoTable.previous.finalY + re, u.setFontSize(14), u.text(y(l("pdf.stock_requirements")), 10, c), c += C, ee = [
    y(l("length")),
    y(l("width")),
    y(l("thickness")),
    y(l("material")),
  ...
  const P = [
    [y(l("pdf.total_parts")), m(v.metadata.totalPartsProduced)],
    [y(l("pdf.part_area")), m(v.metadata.totalPartArea)],
    [y(l("pdf.total_stock_required")), m(v.metadata.totalUsedStock)],
    [y(l("pdf.total_banding_length")), m(v.metadata.totalBandingLength)],
    [y(l("pdf.total_finish_area")), m(v.metadata.totalFinishArea)],
    [y(l("pdf.total_roll_length")), m(v.metadata.totalRollLength)]
  ];
  if (J(u, {
    startY: c,
    body: P,
  ...
  setup(l) {
    ...
    const A = k(!1), I = k("manual"), N = k(""), j = k([]), Q = k(null), B = k(!0), ne = k(null), H = k(!1), a = k(null);
    $e("calculator", () => a.value);
    const g = (e) => {
      A.value && console.log("SmartCut -", ...e);
    }, f = (e) => {
      console.error(`SmartCut - ${e}`);
    }, $ = (e = !0) => {
      H.value = !0, ["checkout", "intro_text", "offcuts", "banding_key"].forEach((r) => ae(r, "block")), et() && ae("pricing_table", "table"), e ? W(!0) : q(), U();
    }, D = (e = !0, n = !0) => {
      H.value = !1, ["checkout", "intro_text", "offcuts", "banding_key", "pricing_table"].forEach((o) => ae(o, "none")), n === !0 ? K() : n === !1 && U(), W();
    }, W = (e = !1) => {
      ae("quantity", "inline-block"), d.quantity.readOnly = e;
    }, q = () => {
      H.value !== !1 && (_e(1), ae("quantity", "none"), d.quantity.readOnly = !0);
    }, K = () => {
      if (!d.cart_button) {
      ...
      t = e.settings;
      const n = Ee(S(window, ["smartcutConfig", "banding_data"])), r = Ee(S(window, ["smartcutConfig", "finish_data"])), o = {
        debug: t.debug,
      ...
      B.value = ze(), B.value ? W(!0) : q(), s.type === "variable" ? D(!1, !1) : $(B.value);
      const i = {
        type: s.type,
        stock: j.value,
        variations: L ?? null,
      ...
      p = w.calculatePrice, _ = w.resetPricing, Ae(() => {
      ...
      const r = e.toFixed(T.number_of_decimals).replace(".", T.decimal_separator), o = xe(r, T.thousands_separator);
      return n ? Fe(o) : o;
    }, Je = (e) => {
      if (e == null) return "";
      typeof e == "string" && (e = parseFloat(e));
    ...
    }, Te = () => t.enable_machining === !0, je = () => !((t == null ? void 0 : t.surcharge_type) === "none" || !(t != null && t.surcharge) || t.surcharge === "0.00"), et = () => !!(t.pricing_strategy === "full_stock_plus_cut_length" || t.pricing_strategy === "full_stock_plus_num_parts" || t.pricing_strategy === "part_area" || t.pricing_strategy === "roll_length" || je() || be("banding") || be("finish") || Te()), tt = (e) => {
      var r;
      let n = (r = e == null ? void 0 : e.attributes) == null ? void 0 : r.attribute_size;
      return n || (n = s == null ? void 0 : s.size), n;
    }, ve = (e) => /\d+x\d+/.test(e), nt = () => {
      window.smartcutConfig.input_fields.forEach((e) => {
        const n = e.replaceAll("_", "-");
        P[n] = document.getElementById(n);
      });
    }, at = (e, n) => {
      Pe(), Q.value = n, g(["variation selected", n.attributes]), j.value = [];
      const r = S(n, ["attributes", "attribute_thickness"]);
      if (s.selected_thickness = r, c.value.forEach((o) => {
        o.t = wt({ v: r });
      }), s.multiple_sizes) {
        const o = tt(n);
      ...
      const r = Yt(v, e, ne.value, Je).split(",")[1], o = new Et();
      ...
      U(), e && (ne.value = Be(e)), ye("smartcut-hardware-price", n ?? 0);
    };
    return Le(() => {
      ...
      fe(M(u), { position: "top-left" }),
      M(E) === "development" && Qt ? (V(), me("div", Nt, [
      ...
      I.value === "formula" && N.value ? (V(), oe(M(O), {
        key: 1,
        ref: "formulapricing",
        "format-price": le,
        url: N.value,
      ...
      I.value === "manual" || I.value === "formula" && N.value ? (V(), oe(Ve, {
        key: 2,
        ref_key: "calculator",
        ref: a,
  ...
  setup(l, { expose: v, emit: O }) {
    const m = ie(() => import("./components-CUGifcIO.js").then((a) => a.F)), u = ie(() => import("./components-CUGifcIO.js").then((a) => a.N)), c = O, E = "", d = k(!1), P = k([]), F = k({
      ...
      finish: null,
      machining: null
    }), p = k("USD"), _ = k("en-US"), t = k("manual"), x = k(""), s = ut(null), L = k(null), R = k();
    $e("calculator", () => R.value);
    const T = (a) => {
      ...
      xt(a), P.value = a.stock, F.value.banding = S(a, ["banding", "pricing"]), F.value.finish = S(a, ["finish", "pricing"]), F.value.machining = S(a, ["machining", "pricing"]), Y(a, ["options", "currency"]) && (p.value = S(a, ["options", "currency"])), Y(a, ["options", "locale"]) && (_.value = S(a, ["options", "locale"]).replace("_", "-")), (Y(a, ["options", "formula", "url"]) || Y(a, ["options", "formula", "spec"])) && (t.value = "formula", x.value = S(a, ["options", "formula", "url"]) ?? null, s.value = S(a, ["options", "formula", "spec"]) ?? null), Ae(() => {
    ...
    return pt(() => {
      ...
      fe(M(u), { position: "top-left" }),
      t.value === "formula" && (x.value || s.value) ? (V(), oe(M(m), {
        key: 0,
        ref: "formulapricing",
        "format-price": Q,
        url: x.value,
  ...
  setup(l, { expose: v }) {
    ...
    return v({
      ...
      checkout: m
    }), (d, P) => E.value ? (V(), oe(dt(E.value), {
      key: 0,
      ref_key: "checkoutRef",
      ref: m
    }, null, 512)) : c.type ? (V(), me("div", Jt, " Invalid component type: " + ft(c.type), 1)) : te("", !0);
  }
