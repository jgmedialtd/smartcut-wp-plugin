import { x as Ke, g as s, w as Me, c as j, d as Ge, S as Ye, h as ze, o as K, e as be, i as Ne, u as se, b as Se, F as Ie, f as ue, j as Fe, t as Ue, J as Xe, M as Ze, a as ce, s as et, n as ke, p as tt } from "./vendor-vue-QD0FJS8o.js";
import { b5 as Re, o as at, bU as u, bf as x, bg as re } from "./result.zod-B0BoTcpc.js";
import { d as Be, E as nt, m as rt, b as ot, p as Le } from "./EcommerceCalculator-ClNeqZHE.js";
import { c as it, a as lt } from "./bandingAxesCache-DMR2pLrK.js";
import { a3 as st, o as J, D as N, l as Z, k as i, i as fe, E as ut, j as oe, h as F, Y as z, n as D, a4 as Ce } from "./vendor-BnSUeZfc.js";
import { _ as ct } from "./Launch-NSW_K3VG.js";
function we(h, p, P) {
  if (!h) return null;
...
function Tt(h) {
  ...
  function ie(t, e) {
    ...
    switch (e.type) {
      case "select":
        return Y(a, e.value);
      case "multiselect":
        return Array.isArray(e.value) ? e.value.some((n) => Y(a, n)) : Y(a, e.value);
      case "range":
  ...
  const te = j(() => {
    ...
    const t = [...te.value];
    return t.sort((e, a) => {
      const n = A(e, V.value), r = A(a, V.value);
  ...
  function De(t) {
    ...
    const a = (f ? U.value : p.value).map((n) => Number(A(n, t))).filter((n) => !isNaN(n));
...
J({
  ...
  settings: N(i(), z()).default({}),
  // Note: HTMLInputElement cannot be validated with Zod, using unknown
  inputs: N(i(), z()).default({}),
  formatting: N(i(), z()).default({}),
  product: N(i(), z()).default({}),
  stock: Z(at).default([]),
  variations: Z(z()).default([]),
  selectedVariation: N(i(), z()).default({}),
...
const mt = {
  ...
  setup(h, { expose: p, emit: P }) {
    const w = ce(() => import("./Catalogue-DqYAER6E.js")), M = ce(() => import("./Configurator-DBmjfmK9.js")), y = ce(() => import("./ProductBrowser-tJgnnab6.js")), m = ce(() => import("./Notices-Dp4_t_6H.js")), g = ce(() => import("./OrderLookup-D6bDX1kY.js")), V = P, b = "production", W = "4.3.0", v = s(!1), c = s([]), f = s({
      banding: null,
      finish: null,
      planing: null,
      machining: null
    ...
    })), B = s("USD"), E = s("en-US"), l = s("metric"), S = s("manual"), O = s(""), $ = et(null), G = s(null), _ = s(), A = s(!1), Y = s([]), ie = s(Be()), ee = s(!1), q = s(!1), te = s(!0), C = s(null), pe = s("calculator"), k = s(null), le = s(null), { inventoryChanged: xe, start: de, acknowledge: Pe } = ft(), Ee = j(() => q.value && !C.value || q.value && C.value?.type === "simple" || !(S.value === "manual" || S.value === "formula" && (O.value || $.value)) ? !1 : A.value ? c.value.length > 0 : !0), _e = j(() => {
      if (!(!ee.value || !k.value))
        return {
          enabled: !0,
    ...
    const R = (e) => {
      ...
      if (C.value = e, R(["Product selected:", e.name, e.type]), e.type === "formula" && e.formulaSpec)
        S.value = "formula", O.value = e.formulaSpec.url ?? "", $.value = e.formulaSpec.spec ?? null, R(["Formula product - loading spec"]);
      ...
      C.value = null, S.value === "formula" && (O.value = "", $.value = null, S.value = "manual");
    }, ye = (e) => {
      rt(e), e.api && (k.value = {
        baseUrl: e.api.baseUrl,
        orgSlug: e.api.orgSlug,
        isCustomDomain: e.api.isCustomDomain
      }), e.stockFilter?.enabled ? (A.value = !0, ee.value = e.stockFilter.serverMode || !1, Y.value = ee.value ? [] : (e.stock || []).map((n) => ({ ...n })), ie.value = e.stockFilter.config || Be(), c.value = [], R(["Stock filter enabled", ee.value ? "(server mode)" : `with ${Y.value.length} available options`])) : (A.value = !1, c.value = e.stock), e.products?.enabled ? (q.value = !0, te.value = e.products.showCategories !== !1, R(["Product browser enabled", te.value ? "with categories" : "without categories"])) : q.value = !1, f.value.banding = x(e, ["banding", "pricing"]), f.value.finish = x(e, ["finish", "pricing"]), f.value.planing = x(e, ["planing", "pricing"]), f.value.machining = x(e, ["machining", "pricing"]), I.value = {
        banding: x(e, ["banding"]) ?? null,
        finish: x(e, ["finish"]) ?? null,
        planing: x(e, ["planing"]) ?? null,
        machining: x(e, ["machining"]) ?? null
      }, re(e, ["options", "currency"]) && (B.value = x(e, ["options", "currency"])), re(e, ["options", "locale"]) && (E.value = x(e, ["options", "locale"]).replace("_", "-")), re(e, ["options", "unitSystem"]) && (l.value = x(e, ["options", "unitSystem"])), re(e, ["config", "configurator", "enabled"]) && e.config.configurator.enabled ? (S.value = "formula", O.value = e.config.configurator.url ?? null, $.value = e.config.configurator.spec ?? null) : (re(e, ["options", "formula", "url"]) || re(e, ["options", "formula", "spec"])) && (S.value = "formula", O.value = x(e, ["options", "formula", "url"]) ?? null, $.value = x(e, ["options", "formula", "spec"]) ?? null), (x(e, ["options", "enable", "debug"]) ?? x(e, ["options", "debug"])) && (v.value = !0), e.api?.wsServer && e.api?.orgId && de(e.api.wsServer, e.api.orgId), ke(() => {
        if (_.value) {
          const n = {
            ...e,
            stock: c.value,
            // Pass flag to indicate stock filter is enabled
            stockFilterEnabled: A.value
          };
          _.value.init(n);
        }
      });
    ...
    return Me(xe, (e) => {
      ...
      pe.value === "order-lookup" && k.value ? (K(), Se(se(g), {
        ...
        A.value && !q.value ? (K(), Se(se(w), {
          key: 1,
          "stock-options": Y.value,
          config: ie.value,
          "format-price": Q,
          colors: { button: "#118ab2", buttonText: "#ffffff" },
        ...
        S.value === "formula" && (O.value || $.value) ? (K(), Se(se(M), {
          key: 3,
          ref: "configurator",
          "format-price": Q,
          url: O.value,
...
}), bt = /* @__PURE__ */ ct(yt, [["__scopeId", "data-v-b9f02dd0"]]), Ot = /* @__PURE__ */ Object.freeze(/* @__PURE__ */ Object.defineProperty({ __proto__: null, default: bt }, Symbol.toStringTag, { value: "Module" }));
export {
  _t as S,
  Ot as V,
  X as g,
