...
import { u as useInputs, a as useNotices, b as useResults, c as useToolTip, d as useSocket, e as useProgress } from "./composables-DsZeVWk_.js";
...
const SAW_SPEC = {
  validValues: {
    stockType: SawConfigSchema.stockTypes,
    cutType: {
      sheet: SawConfigSchema.cutTypes.sheet,
      roll: SawConfigSchema.cutTypes.roll,
      linear: SawConfigSchema.cutTypes.linear
    },
    cutPreference: {
      "sheet.guillotine": SawConfigSchema.cutPreferences.sheet.guillotine,
      "sheet.efficiency": SawConfigSchema.cutPreferences.sheet.efficiency,
      "sheet.beam": SawConfigSchema.cutPreferences.sheet.beam,
      "roll.guillotine": SawConfigSchema.cutPreferences.roll.guillotine,
      "roll.efficiency": SawConfigSchema.cutPreferences.roll.efficiency
    },
    efficiencyOptions: {
      primaryCompression: {
        "sheet.efficiency": SawConfigSchema.efficiencyCompression.sheet.efficiency,
        "roll.efficiency": SawConfigSchema.efficiencyCompression.roll.efficiency
      }
    }
  },
  defaults: {
...
class Saw {
  constructor(data, stockType = "sheet") {
    ...
    this.bladeWidth = convertNumberFormat({ v: data == null ? void 0 : data.bladeWidth }) ?? SAW_SPEC.defaults.bladeWidth;
    this.stockType = (data == null ? void 0 : data.stockType) ?? stockType ?? SAW_SPEC.validValues.stockType[0];
    this.cutType = (data == null ? void 0 : data.cutType) ?? ((_a = SAW_SPEC.validValues.cutType[stockType]) == null ? void 0 : _a[0]);
    this.cutPreference = (data == null ? void 0 : data.cutPreference) ?? ((_b = SAW_SPEC.validValues.cutPreference[`${this.stockType}.${this.cutType}`]) == null ? void 0 : _b[0]);
    if (blankValues.includes(this.cutType)) this.cutType = null;
    if (blankValues.includes(this.cutPreference)) this.cutPreference = null;
    this.stackHeight = (data == null ? void 0 : data.stackHeight) ?? SAW_SPEC.defaults.stackHeight;
    this.guillotineOptions = defaults({}, data == null ? void 0 : data.guillotineOptions, SAW_SPEC.defaults.guillotineOptions);
    this.efficiencyOptions = defaults({}, data == null ? void 0 : data.efficiencyOptions, SAW_SPEC.defaults.efficiencyOptions);
    this.options = defaults({}, data == null ? void 0 : data.options, SAW_SPEC.defaults.options);
    this.issues = [];
    validateSaw(this, data);
  }
  clone() {
    const data = structuredClone(this);
    return new Saw(data);
...
function validateSaw(saw, data) {
  if (valueSet(data == null ? void 0 : data.bladeWidth)) {
    saw.bladeWidth = typeof data.bladeWidth === "string" ? convertNumberFormat({ v: data.bladeWidth }) : data.bladeWidth;
  } else {
    saw.bladeWidth = SAW_SPEC.defaults.bladeWidth;
  }
  if (isNaN$1(saw.bladeWidth) || saw.bladeWidth < 0) {
    createIssue(
      saw,
      "bladeWidth",
      "saw.validation.bladeWidth.invalid",
      { width: saw.bladeWidth ?? "N/A" }
    );
    saw.bladeWidth = SAW_SPEC.defaults.bladeWidth;
  }
  if (!SAW_SPEC.validValues.stockType.includes(saw.stockType)) {
    createIssue(
      saw,
      "stockType",
  ...
  const validCutTypes = get(SAW_SPEC.validValues.cutType, saw.stockType, []);
  if (saw.cutType && !validCutTypes.includes(saw.cutType)) {
    createIssue(
      saw,
      "cutType",
      "saw.validation.cutType.invalid",
      { cutType: saw.cutType, stockType: saw.stockType }
    );
  }
  if (saw.cutType && saw.cutPreference) {
    const validPreferences = get(
      SAW_SPEC.validValues.cutPreference,
      `${saw.stockType}.${saw.cutType}`,
      []
    );
    if (!(validPreferences == null ? void 0 : validPreferences.includes(saw.cutPreference))) {
      createIssue(
        saw,
        ["cutPreference", "cutType"],
        "saw.validation.cutPreference.invalid",
        {
          preference: saw.cutPreference || "N/A",
          cutType: saw.cutType || "N/A",
          stockType: saw.stockType
        }
      );
    }
...
const _hoisted_1$g = ["id", "value", "inputmode", "type", "placeholder", "disabled", "readonly", "required", "min", "max", "aria-label", "aria-invalid", "aria-describedby"];
...
const _sfc_main$e = /* @__PURE__ */ defineComponent({
  ...
  setup(__props, { emit: __emit }) {
    ...
    const convertedValue = computed(() => {
      if (props.type !== "unitDependent" || !props.value) return props.value;
      try {
        return convertNumberFormat({ v: props.value, nf: props.numberFormat });
      } catch {
...
function createShapeLabels() {
  ...
  const lengthTexts = this.selections.shapeWrappers.selectAll("text.shape-text.length");
  if (lengthTexts.empty()) {
    this.selections.shapeLengthText = this.selections.shapeWrappers.append("text").attr("class", "shape-text length").text((shape) => getShapeLengthText.call(this, shape)).classed("hidden", (shape, b, c) => this.isLengthHidden(shape, b, c)).attr("dominant-baseline", "middle").attr("fill", this.config.colors.text.toString()).attr("x", (s) => this.getRectangleCoordinate(s, "x", "center")).attr("y", (s, b, c) => {
      const node = c[b];
      const height = node.getBBox().height;
      return this.getRectangleCoordinate(s, "y", "top", -(height / 2 + 2));
    });
  } else {
    lengthTexts.each((s, i, nodes) => {
      const node = nodes[i];
      const currentX = node.getAttribute("x");
      const currentY = node.getAttribute("y");
      const newX = this.getRectangleCoordinate(s, "x", "center").toString();
      const height = node.getBBox().height;
      const newY = this.getRectangleCoordinate(s, "y", "top", -(height / 2 + 2)).toString();
...
const _hoisted_11$1 = ["disabled", "aria-label", "onClick"];
    ...
    return (_ctx, _cache) => {
      ...
      return openBlock(), createElementBlock(Fragment, null, [
        ...
        unref(env) === "development" && showDevInfo$2 ? (openBlock(), createElementBlock("div", _hoisted_1$6, [
          ...
          createBaseVNode("div", null, "options.orientationModel: " + toDisplayString(options.orientationModel), 1)
        ])) : createCommentVNode("", true),
...
const CheckoutCalculator = /* @__PURE__ */ _export_sfc(_sfc_main$8, [["__scopeId", "data-v-a2de53a3"]]);
const _sfc_main$7 = {
  name: "ObjectViewer",
  props: {
    data: {
    ...
    return (_ctx, _cache) => {
      ...
      return openBlock(), createElementBlock(Fragment, null, [
        env.value === "development" && showDevInfo ? (openBlock(), createElementBlock("div", _hoisted_1$1, [
          ...
          specification.value ? (openBlock(), createBlock(unref(ObjectViewer2), {
            key: 0,
...
//# sourceMappingURL=components-D88dKAyh.js.map
