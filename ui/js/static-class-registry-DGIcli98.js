import { G as gs, H as Ue, J as ae, K as Ai, L as ms, M as Ss, N as ys, O as ws, P as ct, Q as bs, R as $s, T as xs, U as Is, V as ks, W as mt, X as Ts, Y as Cs, _ as B, $ as St, a0 as me, a1 as Ps, a2 as Me, a3 as Es, a4 as D, a5 as _, a6 as we, a7 as P, a8 as ut, a9 as k, aa as Li, ab as F, ac as j, ad as he, ae as O, af as Ce, ag as v, ah as b, ai as be, aj as vs, ak as yt, al as Ht, am as Ae, an as J, ao as Oi, ap as ee, aq as je, z as Se, ar as oe, as as ft, at as X, au as Le, av as qe, aw as wt, ax as Ds, ay as Mi, az as Rs, aA as As, aB as ue, aC as _e, aD as $e, aE as Pe, aF as xe, aG as Ls, aH as qi, aI as Fi, aJ as Gi, aK as Ge, aL as Os, aM as Ni, aN as Bi, aO as Ms, aP as bt, aQ as qs, aR as ce, aS as ot, aT as Hi, aU as Fs, aV as Qt, aW as Gs, aX as Ns, aY as Bs, aZ as Hs, a_ as Zt, a$ as zs, b0 as $t, b1 as xt, b2 as Ws, b3 as Us, b4 as js, F as ze, S as Ye, b5 as _s, b6 as It, b7 as kt, b8 as Ys, b9 as Xs, ba as Oe, bb as Vs, bc as Ks, bd as ht, be as dt, bf as Js, bg as R, bh as ei, bi as ti, bj as ii, bk as si, bl as ni, bm as ri, bn as oi, bo as ai, bp as li, bq as ci, I as ui, br as fi, bs as hi, bt as di } from "./components-Bf39GJww.js";
import { b as de, n as ne, _ as Xe, o as zi, c as pi, s as ke, a as pt, H as te, I as W, J as ie, K as De, L as Qs, M as z, N as Ve, O as U, P as Zs } from "./vendor-zod-PVURV34D.js";
import { m as Wi, a as en } from "./vendor-lodash-CYGhmG8H.js";
import { k as Y } from "./vendor-tCV_BFOF.js";
  ...
  addChild(t) {
    ...
    if (!(t.parent && t.parent.autoId === this.autoId)) {
      ...
      if (Array.isArray(this.children) || (this.children = []), t.parent && t.parent !== this) {
        ...
        me() && console.log(`[PARENT CHANGE] Moving ${t.autoId} from parent ${i.autoId} to ${this.autoId}`);
      }
...
const Re = Y("logs"), Tt = Y("cuts"), Je = Y("groups"), Qe = Y("guillotine"), Ct = Y("results"), Pt = Y("scoring"), Et = Y("firstShapes"), vt = Y("calculations"), Dt = Y("subset"), Rt = Y("secondRun"), At = Y("stack"), Ze = Y("errors"), Lt = Y("allStock"), et = Y("reset"), tt = Y("swarm");
Ze.color = 1;
Je.color = 14;
Qe.color = 159;
et.color = 11;
...
function Be({
  ...
  for (let m = 0; m < h; m++) {
    ...
    if (l = jn.call(this, l), l.final && l.segment?.parent?.hasHeadCut && l.segment.phase === 1 && e.type === "far" && l.isHeadCut === !1) {
      const $ = Ji(l.cutInfo), { cutAxis: I, cutDimension: y, positionDimension: E, positionAxis: A } = $;
      at({
        parentSegment: e,
        stock: t,
        cutDirection: y,
    ...
    if (l = Mn.call(this, l), l.final && t.cutType === "beam" && m === 0 && (e.type !== "far" || Qi(l)) && (l = qn.call(this, l)), l = Fn.call(this, l), En.call(this, l) === !0) {
...
function ye(e, t, i) {
  ...
  if (_(t)) {
    if (t.cutType === "efficiency" ? (w = t.primaryCompression ?? "w", p = ft(
      ...
      w,
      a.bladeWidth
    ), p = p / t[w === "x" ? "l" : "w"]), t.cutType === "guillotine" && (r = e.filter((C) => C.added && Xt(C) === 1).length), o = I / $, me() && v([() => b(o).to.be.lessThanOrEqual(1)]), t?.saw?.stockType === "roll") {
      const C = Ce(l, "x");
      s = C.x + C.l;
    }
  } else if (j(t)) {
    ...
    o = I / $;
  const y = l.reduce((C, q) => C + (q?.bestScore?.total ?? 0), 0), E = X(e, t).length + f, A = X(e, a).length + f, M = l.map((C) => C.compress());
  return {
...
function br(e, t) {
  return e.score.efficiency > t.score.efficiency ? -1 : e.score.efficiency < t.score.efficiency ? 1 : e.type !== "evo" && t.type === "evo" ? -1 : e.type === "evo" && t.type !== "evo" ? 1 : e.shapes?.length < t.shapes?.length ? -1 : e.shapes?.length > t.shapes?.length ? 1 : e.score.totalShapeScore > t.score.totalShapeScore ? -1 : e.score.totalShapeScore < t.score.totalShapeScore ? 1 : Se.ID(e.stock, t.stock);
}
function $r(e, t) {
  ...
  const r = ls(e, t);
  return r !== 0 ? r : e.score.firstShapeArea > t.score.firstShapeArea ? -1 : e.score.firstShapeArea < t.score.firstShapeArea ? 1 : Se.ID(e.stock, t.stock);
}
function xr(e, t) {
  ...
  if (e.score[this.config.successMetric] < t.score[this.config.successMetric])
    return 1;
  const r = ls(e, t);
  return r !== 0 ? r : e.type !== "evo" && t.type === "evo" ? -1 : e.type === "evo" && t.type !== "evo" ? 1 : e.score.firstShapeArea > t.score.firstShapeArea ? -1 : e.score.firstShapeArea < t.score.firstShapeArea ? 1 : e.score.totalShapeScore > t.score.totalShapeScore ? -1 : e.score.totalShapeScore < t.score.totalShapeScore ? 1 : Se.ID(e.stock, t.stock);
}
function cs(e) {
  if (!e) return e;
  const t = { ...e };
...
function Ne({
  ...
  for (const l of e.shapes)
    if (wt(l) || Ds(l)) {
      ...
      if (!h) {
        me() && (console.warn(`[updateShapeListFromBestResult] Shape ${l.autoId || l.id} from result not found in shapeList!`), console.warn(`  Available autoIds in shapeList: ${t.slice(0, 5).map((f) => f.autoId).join(", ")}...`));
        continue;
      }
      me() && (O(h) || console.warn(`Shape ${l.autoId} in shapeList is not a live instance!`)), h.update({ ...l, validationMode: "none" }), O(h) && l.group && l.group.inGroup !== void 0 && (h.group.inGroup = l.group.inGroup), h.stock = o, s.push(h);
    }
  v([
    () => b(e.shapes.every((l) => {
      if (!wt(l)) return !0;
  ...
  ]), me() && (s.every((h) => t.find((f) => f === h)) || console.warn("Some updated shapes are not exact references from shapeList - possible instance copying!")), Mi({
    shapes: s
  ...
  try {
    ...
    for (const y of t) {
      ...
      const G = M.length > 0 ? M.reduce((L, N) => {
        const K = L[r] + L[s], fe = N[r] + N[s];
        return k.equalTo(K, fe) ? L[s] > N[s] ? L : N : K > fe ? L : N;
      }) : M[0];
      if (!G) continue;
      const C = G[r] + G[s], q = M.length > 0 ? M.reduce((L, N) => {
        const K = L[o] + L[a], fe = N[o] + N[a];
        return k.equalTo(K, fe) ? L[a] > N[a] ? L : N : K > fe ? L : N;
      }) : M[0];
      if (!q) continue;
      let V = q[o] + q[a];
      p > V && (V = p);
...
async function Fr(e, t, i, n) {
  ...
  for (let u = 0; u < o.length; u++) {
    let d = function(L, N, K, fe) {
      return L[N] + L[K] + fe;
    }, p = function(L, N, K, fe, Jt) {
      return Jt > 0 ? Jt - N[K] : L[fe] - N[K];
    };
    const S = o[u], g = u > 0 ? oe(this.shapeList, e) : i, w = X(t, e);
    if (!w.length) {
...
function _r(e, t) {
  ...
  function r(a) {
    return e.some((c) => a.parentId === c.parentId && !c.added) && Qt(a, i) && a.willItFit(t);
  }
  const s = e.some((a) => Qt(a, i) && a.willItFit(t) && a.l >= n || a.w >= n);
...
const Te = {
  ...
  createFirstShapeGroups: function(e, t) {
    ...
    const i = X(e, t).filter((o) => O(o)), n = ["l", "w"], r = /* @__PURE__ */ new Map();
    for (const o of n)
      Ni({
        shapes: i,
        container: t,
...
function Vr(e, t) {
  ...
  const i = X(this.shapeList, t, "filter", null), n = we(i, null, !0);
...
Q.index({ organisation: 1, name: 1 });
Q.index({ organisation: 1, material: 1 });
Q.index({ organisation: 1, ecommerce: 1, name: 1 });
Q.index({ organisation: 1, ecommerce: 1, material: 1 });
Q.index({ organisation: 1, ecommerce: 1, _id: -1 });
Q.index({ organisation: 1, ecommerce: 1, l: 1, w: 1 });
Q.index({ organisation: 1, ecommerce: 1, cost: 1 });
Q.index({ organisation: 1, ecommerce: 1, q: 1 });
Q.index({ organisation: 1, isOffcut: 1, material: 1 });
Q.index({ name: "text", material: "text", category: "text", tags: "text" });
Q.plugin(Ui);
const io = lt.model(
  "stock",
  Q,
...
class Ri extends Xs {
  ...
  init() {
    ...
    }), this.shapeList.push(...t), this.shapeList.sort(Se.ID));
  ...
  async run() {
    ...
    async function i(s, o) {
      ...
      if (T("allStock", `calculating stock ${s.id} with ${a} calculation`), !Ie.types.includes(a))
    ...
    function n(s, o) {
      ...
      return c.sort(Se.ID), T("allStock", `refined stock ${c.map((l) => l.id).join()}`), c;
    }
  ...
  async allStock(t, i) {
    ...
    for (let h = Oe(this.stockList, !1); h.length > 0; h = Oe(this.stockList, !1)) {
      ...
      if (d.length && d.forEach((g) => {
        const w = g.clone(Js(this.stockList, g));
        this.stockList.push(w), r ? l = w.parentId : l = !1, S++;
      }), S ? (this.stockList.sort(Se.AA), this?.job && this.job.updateProgress({
        socketId: this.socketId,
        type: "addStock",
