...
define('SMARTCUT_CURRENT_VERSION', '3.1.36'); // This needs to be kept in sync with the version above.
...
//ensuring script has type module
...
//fallback if wp_script_attributes fails
add_action('script_loader_tag', function ($tag, $handle) {

	if ($handle !== 'smartcut-checkout') return $tag;
...
function create_product_template_tools_page()
{
	...
	foreach ($template_products as $name => $product_data) {
		...
		if ($product_post) {
			...
			if (!isset($product_data['is_extra']) && !isset($product_data['is_machining'])) {
			...
			if (isset($product_data['finish_types'])) $content .= add_table_row(['Finish types', $product_data['finish_types']]);
			...
			//category
			if ((!isset($product_data['is_extra']) || $product_data['is_extra'] === false) && (!isset($product_data['is_machining']) || $product_data['is_machining'] === false)) {
			...
			$settings = [
				...
				'finish_types' => isset($product_data['finish_types']) ? $product_data['finish_types'] : '',
			...
			if ($variable_product) {
					...
					if (isset($product_data['thickness']) && isset($product_data['size']) && is_array($product_data['thickness']) && is_array($product_data['size'])) {
						foreach ($product_data['thickness'] as $thickness) {
							foreach ($product_data['size'] as $size) {
								...
								$variation->set_regular_price(number_format($size === 'Cut to size' ? $cut_to_size_price : $price, 2, '.'));
								$variation->save();
								$price = $size === 'Cut to size' ? 1 : $price + 100;
							}
						}
					}
				}
				...
				if (isset($product_data['attributes']) && count($product_data['attributes']) > 0) {
					...
					create_product_variations($product_id, $product_data['attributes'],  isset($product_data['price']) ? $product_data['price'] : 100);
				}
			}
...
function create_health_check_tools_page()
{
	...
	$query = new \WP_Query($args);
	$number_products = $query->post_count;
